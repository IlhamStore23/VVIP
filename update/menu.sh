...
UPDATE="https://raw.githubusercontent.com/IlhamStore23/VVIP/ipuk/update/update.sh"
GREEN='\033[0;32m'
RED='\033[0;31m'
ipsaya=$(wget -qO- ipinfo.io/ip)
data_server=$(curl -v --insecure --silent https://google.com/ 2>&1 | grep Date | sed -e 's/< Date: //')
date_list=$(date +"%Y-%m-%d" -d "$data_server")
data_ip="https://raw.githubusercontent.com/IlhamStore23/izinvps/ipuk/ip"
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "      \033[0;36mTelegram${NC} t.me/IlhamStore23"
    echo -e "      ${GREEN}WhatsApp${NC} wa.me/627824016438"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    exit
  fi
}
...
Exp=$(curl -sS https://raw.githubusercontent.com/IlhamStore23/izinvps/ipuk/ip | grep $MYIP | awk '{print $3}')
fi
...
echo -e "  ${BLUE}• ${GREEN}Developer           ${NC}= IlhamStore Tunneling ${NC}"
echo -e "$COLOR1┌────────────────────────────────────────────────────────────┐${NC}"
